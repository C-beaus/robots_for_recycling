...
def main():
    ...
    pandaController.add_table()  # Add Table in the workspace
    pandaController.add_vbar()   # Add vertical steel bar in the workspace
    pandaController.add_hbar()   # Add horizontal steel bar in the workspace
    pandaController.add_camera() # Add camera in the workspace
    pandaController.set_def_pos() # Move the robot to home configuartion

    loaded = np.array([0.2, 0.2, 0.4])  # Coodrinates are given wrt to camera    (End effector is not considered as a connected part) 
