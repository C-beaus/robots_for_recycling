...
class GraspRectangles:
    ...
    def __getattr__(self, attr):
        """
        Access attribute or method from GraspRectangle for all elements.
        """
        if hasattr(GraspRectangle, attr):
            # Check if it’s a method, then call it on each item.
            if callable(getattr(GraspRectangle, attr)):
                return lambda *args, **kwargs: [getattr(gr, attr)(*args, **kwargs) for gr in self.grs]
            else:
                # It's an attribute, return its value from each GraspRectangle.
                return [getattr(gr, attr) for gr in self.grs]
        else:
            raise AttributeError(f"Couldn't find attribute or function '{attr}' in GraspRectangle.")
