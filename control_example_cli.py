...
#from robots_for_recycling.srv import ClassifySrv, GraspSrv, CameraSrv
from robots_for_recycling.srv import FinRayGraspLocationSrv
...
#from grasp_location_interfaces.srv import *#GraspLocationFunction
...
# Terminal 3: cd RoboticRecycling2023/RBE595/   --> source devel/setup.bash   --> rosrun robots_recycling control_example_cli.py
...
#self.grip_location = rospy.Publisher('/fin_ray_gripper_command_publisher', Bool, queue_size=5)
...
class ExpClass:
    def __init__(self):
        ...
        #last element for srv cli
        #self.service = rospy.Service('camera_service', CameraSrv, self.capture_frames_callback)

        self.service = rospy.Service('fin_ray_grasp_location_service', FinRayGraspLocationSrv, self.fin_ray_grasp_node)
    ...
    #def call_fin_ray_grasp_location_service(self):
        
        # Wait for the service to become available
       # rospy.wait_for_service('fin_ray_grasp_location_service')
      #  try:

       #     get_fin_ray_grasp_location = rospy.ServiceProxy('fin_ray_grasp_location_service', FinRayGraspLocationSrv)

      #      # Call the service
     #       rospy.loginfo("Calling the grasp location service to get the grasp location ...")
        #     response = get_fin_ray_grasp_location()

        #     # Check and handle the response
        #     if response:
        #         rospy.loginfo("fin_ray_grasp_location received. Ready For Fin Ray picking")
        #         return response.fin_ray_grasp_location
        #     else:
        #         rospy.logwarn("Capture of fin_ray_grasp_location was unsuccessful.")
        #         return None

        # except rospy.ServiceException as e:
        #     rospy.logerr(f"Service call to the grasp location service failed: {e}")
...
#grasp_location = [0, 1, 2, 3, 4, 5]
    def fin_ray_grasp_node(self, grip_location):
        '''
