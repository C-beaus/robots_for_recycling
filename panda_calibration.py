...
def main():
    ...
    while True:
        ...
        print(f"x: {wpose.position.x}, y: {wpose.position.y}, z: {wpose.position.z}, orientation: {wpose.orientation}")
    ...
    #########################################
    # loaded = np.array([0.0744, -0.0944, 0.7221, 1])
    ...
    # was commented out
    # loaded = np.array([0, 0, 0.54])  # Coodrinates are given wrt to camera    (End effector is not considered as a connected part) 
    ...
    # pandaController.execute_traj_start(start) 

    # object_pose = pandaController.scene.get_object_poses(["block"])['block']
    # object_position = pandaController.scene.get_object_poses(["block"])['block'].position
    # object_orientation = pandaController.scene.get_object_poses(["block"])['block'].orientation

    # # Pre Grasp Position
    # print("Moving to Pre Grasp Position location")
    # pandaController.execute_traj_start([ 0.515, 0, 0.16 + (0.042/2) + 0.1])    # Move the robot to given location
    # # pandaController.set_gripper_distance(0.08)
    # # ipdb.set_trace()
    # # Grasp Position
    # print("Moving to Grasp Position location")
    # pandaController.execute_traj_start([ 0.515, 0, 0.16 + (0.042/2)])    # Move the robot to given location
    # pandaController.scene.attach_object("block", "panda_hand", touch_links=["panda_hand", "panda_leftfinger", "panda_rightfinger"])
    # # pandaController.set_gripper_distance(0.0302)
    # # ipdb.set_trace()
    # # Post Grasp Position
    # print("Moving to Post Position location")
    # pandaController.execute_traj_start([ 0.4, 0, 0.16 + (0.042/2) + 0.1])  
    # # ipdb.set_trace()

    # loaded = np.array([-0.2, 0.2, 0.59])  # Coodrinates are given wrt to camera    (End effector is not considered as a connected part) 
    # start = pandaController.tf_cam_to_panda([loaded[0], loaded[1], loaded[2]])[:-1]   # Convert coordinates to Panda Frame
    # pandaController.execute_traj_start(start) 
    # # pandaController.set_gripper_distance(0.08)
    # pandaController.scene.remove_attached_object()
